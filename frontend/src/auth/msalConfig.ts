...
import { getAuthConfig, validateEnvironmentConfig, getCurrentDomainConfig, ENTRA_ID_CONFIG } from '../config/environment';
...
// Create dynamic MSAL configuration that adapts to runtime domain
export const createMsalConfig = (): Configuration => {
  // Get domain-aware configuration at runtime
  const domainConfig = getCurrentDomainConfig();
  const authConfig = getAuthConfig();
  
  console.info('🔐 Building MSAL configuration for domain:', domainConfig.domain);
  ...
  return {
    ...
    cache: {
      ...
      storeAuthStateInCookie: true, // Required for some browser environments  // Disable cookies to avoid domain-related issues  
