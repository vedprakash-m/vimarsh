...
class ProgressivePersonalizationService:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.info("✅ Using Phase 2 database service for personalization")
        except ImportError:
    ...
    async def initialize_user_personalization(
        ...
        try:
            ...
            # Store profile in database if available
            database_stored = False
            if self.database_service:
                try:
                    stored = await self.database_service.store_user_preferences(profile)
                    if stored:
                        self.logger.info(f"💾 Stored user preferences in database: {user_id}")
            ...
            self.logger.info(
                f"✅ Initialized personalization for user {user_id} "
    ...
    async def get_user_preferences(self, user_id: str) -> Optional[UserPreferences]:
        """Get user preferences with database fallback."""
        ...
        try:
            ...
            if self.database_service:
                try:
                    preferences = await self.database_service.get_user_preferences(user_id)
                    if preferences:
                        # Cache the result
                        self._user_profiles[user_id] = preferences
                        self.logger.info(f"📚 Retrieved user preferences from database: {user_id}")
                        return preferences
                except Exception as e:
