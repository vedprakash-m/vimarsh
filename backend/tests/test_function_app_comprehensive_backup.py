...
class TestFunctionAppCore:
    ...
    def mock_spiritual_guidance_service(self):
        ...
        @pytest.mark.asyncio
    async def test_spiritual_guidance_endpoint_success(self, function_app_test_config):
        """Test successful spiritual guidance endpoint response"""
        
        # Create a mock request for the guidance endpoint
        ...
        # Mock the guidance endpoint instead of non-existent SpiritualGuidanceService
        with patch('function_app.enhanced_llm_service') as mock_llm_service:
            mock_llm_service.generate_response = AsyncMock(return_value={
                'content': 'Dharma represents righteous duty and moral law.',
                'personality_id': 'krishna',
                'citations': ['Bhagavad Gita 2.47'],
                'metadata': {'response_source': 'ai_generated'}
            })
            
            # Import the actual guidance endpoint
