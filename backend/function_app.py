...
# ==============================================================================
# VOICE TTS API ROUTE (Azure Speech Service)
# ==============================================================================
...
try:
    ...
    if azure_speech_available:
        logger.info("🎙️ Azure Speech Service initialized successfully")
    else:
    ...
    logger.warning(f"⚠️ Azure Speech Service initialization failed: {e}")


@app.route(route="voice/synthesize", methods=["POST", "OPTIONS"])
async def voice_synthesize(req: func.HttpRequest) -> func.HttpResponse:
    ...
    Request body:
    {
        ...
        "personality": "krishna",  // Optional, defaults to "krishna"
        "format": "mp3"  // Optional: mp3, mp3-hd, wav, ogg
    }
    ...
    try:
        ...
        try:
            ...
            return func.HttpResponse(
                json.dumps({
                    ...
                    "message": "Expected JSON with 'text' field"
                }),
                status_code=400,
                headers=get_cors_headers()
            )
        ...
        if audio_data is None:
            return func.HttpResponse(
                json.dumps({
                    ...
                    "message": "Unable to generate audio for the provided text"
                }),
                status_code=500,
                headers=get_cors_headers()
            )
        ...
        logger.error(f"❌ Voice synthesis configuration error: {str(e)}")
        return func.HttpResponse(
            json.dumps({
...
@app.route(route="voice/info", methods=["GET"])
async def voice_info(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Format for API response (remove duplicates by using primary names)
        personalities = {}
        for name, config in voice_configs.items():
            # Skip alias names (shorter versions)
        ...
        logger.error(f"❌ Voice info error: {str(e)}")
        return func.HttpResponse(
            json.dumps({
                "error": "Failed to get voice info",
                "message": str(e)
            }),
            status_code=500,
            headers=get_cors_headers()
