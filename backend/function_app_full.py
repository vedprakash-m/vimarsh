...
try:
    from core.logging import get_logger, LogContext, PerformanceMetrics
...
try:
    ...
    logger.warning(f"⚠️ Token tracking not available: {e}")
...
async def user_budget_status(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if TOKEN_TRACKING_AVAILABLE:
            # Import was done at startup, so just use token_tracker and budget_validator directly
